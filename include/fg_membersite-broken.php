...
class FGMembersite
{
    ...
    function LinkToCompany()
    {
        ...
        if(!$this->ValidateCompanyLink($companyID))
        {
            return false;
        }
    ...
    {
        return isset($_SESSION['user_company'])?$_SESSION['user_company']:'';
    }
    function UserFirstLogin()
    {
        return isset($_SESSION['user_first_login'])?$_SESSION['user_first_login']:'';
    } 
    function LogOut()
    {
        session_start();
    ...
    function CheckCompanyLinkInDB(&$clinicID)
    {
        if(!$this->DBLogin())
        {
        ...
        $_SESSION['user_company']  = $row['tax_id'];
    ...
    function UpdateDBRecForConfirmation(&$user_rec)
    {
        ...
        $result = mysql_query("Select name, email,company,first_login from $this->tablename where confirmcode='$confirmcode'",$this->connection);   
        if(!$result || mysql_num_rows($result) <= 0)
        {
    ...
    function SendUserWelcomeEmail(&$user_rec)
    {
        ...
        $mailer->Subject = "Bem Vindo a ".$this->sitename;
    ...
    function ValidateCompanyLink(&$CompanyID))
    {
        ...
        if(!isset($_SESSION)){ session_start(); }
        if(!$this->CheckCompanyLinkInDB($clinicID))
        {
            return false;
        }
        return true;
